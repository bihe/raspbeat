...
UserService.prototype = (function() {
  ...
  return {
    ...
    getTokenFromUser: function(id) {
      ...
      User.findById(id).exec(function (err, user) {
        ...
        if(user.token && user.token !== '') {
          ...
          Iron.unseal(user.token, config.application.secret, Iron.defaults, function(err, unsealed) {
            if(err) {
              return deferred.reject(err);
            }
    ...
    setToken: function(userid, token) {
      ...
      Iron.seal(token, config.application.secret, Iron.defaults, function(err, sealed) {
        if(err) {
          return deferred.reject(err);
        }
